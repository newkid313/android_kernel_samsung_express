...
static struct acpu_level * __init select_freq_plan(void)
{
	...
	if (cpu_is_msm8960()) {
		...
		if (global_sec_pvs_value == 0xfafa) {
			v2 = acpu_freq_tbl_8960_kraitv2_slow;
			boost_vdd_core(v2);
			pr_alert("ACPU PVS: pvs[%d]:fmax[%d]-r slow + boost\n", pvs, fmax);
		}
#else
		if ((pvs == 0x3) && (global_sec_pvs_value == 0xfafa)) {
			v2 = acpu_freq_tbl_8960_kraitv2_nom;
			pr_alert("ACPU PVS: pvs[%d]:fmax[%d]-r nom\n", pvs, fmax);
		}
		else if ((pvs == 0x1) && (global_sec_pvs_value == 0xfafa)) {
			v2 = acpu_freq_tbl_8960_kraitv2_slow;
			pr_alert("ACPU PVS: pvs[%d]:fmax[%d]-r slow\n", pvs, fmax);
		}
#endif
